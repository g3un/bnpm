...
def setup_bn(plugin_dir: Path | None = None) -> Path:
    plugin_dir = (
        ...
        if plugin_dir
        else get_config().bn_user_plugin_dir
    )
...
def setup_bnpm_venv(venv_path: Path | None = None) -> Path:
    venv_path = (
        venv_path.expanduser().resolve() if venv_path else get_config().bnpm_venv_dir
    )
...
def install_plugin_files(output: Path) -> Path:
    ...
    shutil.copy2(
        package_root / "_binaryninja" / "plugin_init.py", output / "__init__.py"
    )
